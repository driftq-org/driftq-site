...
const UseCasesPage = () => (
  <DocsShell currentPath="/docs/use-cases">
    <div className="space-y-4">
      ...
      <p>
        DriftQ is most valuable when you have <strong>slow, flaky, or expensive work</strong> (LLMs,
        tool calls, ingestion) and you need it to be{" "}
        <strong>durable, retryable, and debuggable</strong> — without turning application code into a
      ...
      <div className="not-prose my-6 grid gap-4 md:grid-cols-3">
        ...
        <Callout tone="bad" title="Probably overkill when…">
          <ul className="mt-2 list-disc space-y-1 pl-5">
            ...
            <li>duplicates or loss are acceptable</li>
            <li>you don’t need retries, DLQ, or replay</li>
          </ul>
        </Callout>

        <Callout tone="neutral" title="One-line pitch">
          DriftQ turns unreliable “background work” into a consistent workflow system:
          idempotency-safe, retryable, observable, and resumable.
        </Callout>
      </div>

      <h2>One common application pattern</h2>
      <p>
        A typical setup looks like this: an API layer stays the <em>front door</em> (auth, validation,
        quotas). DriftQ becomes the <em>work backbone</em>. Workers execute long-running or failure-
        prone jobs. The frontend subscribes to progress and results (SSE, WebSocket, polling — your
        call).
      </p>
      ...
      <div className="not-prose my-6 grid gap-4 md:grid-cols-2">
        <Callout tone="neutral" title="Typical flow">
          <ol className="mt-2 list-decimal space-y-1 pl-5">
            <li>Client triggers work via an API request</li>
            <li>The API validates and enqueues a message into DriftQ</li>
            <li>Workers consume and execute the pipeline</li>
            <li>Workers ack success / nack failure with retries and DLQ</li>
            <li>The UI reconnects and resumes using the same run ID</li>
          </ol>
        </Callout>
      ...
      <p>
        If your app is tiny, tasks are sub-second, and you can tolerate occasional loss or
        duplication, you’ll move faster with something simpler. DriftQ is infrastructure — it must
        earn its place.
      </p>

      <div className="not-prose my-6">
        <Callout tone="warn" title="Blunt adoption advice">
          Don’t integrate DriftQ everywhere at once. Pick one high-pain workflow (ingestion,
          webhooks, notifications, long-running jobs). Prove reliability and cost savings. Then
          expand.
        </Callout>
      </div>
    </div>
